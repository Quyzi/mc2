...
pub trait StorageShard<'b, B, K> {
    ...
    fn filter_list<F>(&self, f: F) -> Result<impl Iterator<Item = K>, Self::Error>
        where F: FnMut(K) -> Result<K, Self::Error>;
    ...
    fn compare_swap(&self, key: K, old: Option<B>, new: Option<B>) -> Result<Option<B>, Self::Error>;
    fn update_fetch<F>(&self, key: K, f: F) -> Result<Option<B>, Self::Error>
        where F: FnMut(Option<B>) -> Option<B>;
}
...
pub trait StorageTransaction<'b, B, K> {
    ...
    fn get_many(&self, keys: Vec<K>) -> Box<dyn Future<Output = Vec<(u64, Result<B, Self::Error>)>>>;
    ...
    fn delete_many(&self, keys: Vec<K>) -> Box<dyn Future<Output = Vec<Result<(K, Option<B>), Self::Error>>>>;

    fn compare_swap(&self, key: K, old: Option<B>, new: Option<B>) -> Box<dyn Future<Output = Result<Option<B>, Self::Error>>>;
    fn update_fetch<F>(&self, key: K, f: F) -> Box<dyn Future<Output = Result<Option<B>, Self::Error>>>
        where F: FnMut(Option<B>) -> Option<B>;
}
